...
public interface ISystems<T> : IDisposable
{
    /// <summary>
...
/// <summary>
///     A basic implementation of a <see cref="ISystems{T}"/>.
/// </summary>
/// <typeparam name="W">The world type.</typeparam>
/// <typeparam name="T">The type passed to the <see cref="ISystems{T}"/> interface.</typeparam>
public abstract class BaseSystem<W, T> : ISystems<T>
{
...
/// <summary>
///     A group of <see cref="ISystems{T}"/>'s to organize them.
///     They will run in order.
/// </summary>
/// <typeparam name="T">The type passed to the <see cref="ISystems{T}"/>.</typeparam>
public class Group<T> : ISystems<T>, IEnumerable<ISystems<T>>
{
#if !ARCH_METRICS_DISABLED
    ...
    /// <summary>
    ///     Creates an instance with an array of <see cref="ISystems{T}"/>'s that will belong to this group.
    /// </summary>
    /// <param name="name">A unique name to identify this group</param>
    /// <param name="systems">An <see cref="ISystems{T}"/> array.</param>
    public Group(string name, params ISystems<T>[] systems)
        : this(name, (IEnumerable<ISystems<T>>)systems)
    {
    }

    /// <summary>
    ///     Creates an instance with an <see cref="IEnumerable{T}"/> of <see cref="ISystems{T}"/>'s that will belong to this group.
    /// </summary>
    /// <param name="name">A unique name to identify this group</param>
    /// <param name="systems">An <see cref="IEnumerable{T}"/> of <see cref="ISystems{T}"/>.</param>
    public Group(string name, IEnumerable<ISystems<T>> systems)
    {
        Name = name;
    ...
    /// <summary>
    ///     Adds several new <see cref="ISystems{T}"/>'s to this group.
    /// </summary>
    /// <param name="systems">An <see cref="ISystems{T}"/> array.</param>
    /// <returns>The same <see cref="Group{T}"/>.</returns>
    public Group<T> Add(params ISystems<T>[] systems)
    {
    ...
    /// <summary>
    ///     Adds an single <see cref="ISystems{T}"/> to this group by its generic.
    ///     Automatically initializes it properly. Must be contructorless.
    /// </summary>
    /// <typeparam name="G">Its generic type.</typeparam>
    /// <returns>The same <see cref="Group{T}"/>.</returns>
    public Group<T> Add<G>() where G : ISystems<T>, new()
    {
        return Add(new G());
    }

    /// <summary>
    ///     Adds an single <see cref="ISystems{T}"/> to this group.
    /// </summary>
    /// <param name="system"></param>
    /// <returns></returns>
    public Group<T> Add(ISystems<T> system)
    {
    ...
    /// <returns></returns>
    public G Get<G>() where G : ISystems<T>
    {
    ...
    /// <summary>
    ///     Finds all <see cref="ISystems{T}"/>s which can be cast into the given type.
    /// </summary>
    /// <typeparam name="G">The Type.</typeparam>
    /// <returns></returns>
    public IEnumerable<G> Find<G>() where G : ISystems<T>
    {
        foreach (var item in _systems)
        {
            if (item.System is G sys)
    ...
    /// <summary>
    ///     Initializes all <see cref="ISystems{T}"/>'s in this <see cref="Group{T}"/>.
    /// </summary>
    ...
    /// <summary>
    ///     Runs <see cref="ISystems{T}.BeforeUpdate"/> on each <see cref="ISystems{T}"/> of this <see cref="Group{T}"/>..
    /// </summary>
    /// <param name="t">An instance passed to each <see cref="ISystems{T}.Initialize"/> method.</param>
    ...
    /// <summary>
    ///     Runs <see cref="ISystems{T}.Update"/> on each <see cref="ISystems{T}"/> of this <see cref="Group{T}"/>..
    /// </summary>
    /// <param name="t">An instance passed to each <see cref="ISystems{T}.Initialize"/> method.</param>
    ...
    /// <summary>
    ///     Runs <see cref="ISystems{T}.AfterUpdate"/> on each <see cref="ISystems{T}"/> of this <see cref="Group{T}"/>..
    /// </summary>
    /// <param name="t">An instance passed to each <see cref="ISystems{T}.Initialize"/> method.</param>
    ...
    /// <summary>
    ///     Disposes this <see cref="Group{T}"/> and all <see cref="ISystems{T}"/>'s within.
    /// </summary>
    public void Dispose()
    {
    ...
    /// <inheritdoc/>
    public IEnumerator<ISystems<T>> GetEnumerator()
    {
    ...
    /// <summary>
    ///     The struct <see cref="SystemEntry"/> represents the given <see cref="ISystems{T}"/> in the <see cref="Group{T}"/> with all its performance statistics.
    /// </summary>
    private readonly struct SystemEntry : IDisposable
    {
        public readonly ISystems<T> System;
        ...
        public SystemEntry(ISystems<T> system
#if !ARCH_METRICS_DISABLED
                , Meter meter
#endif
            )
