...
var i = {};
function a(e, t = !1) {
	if (!(t && i[e])) return i[e] = !0, `${e} needs to be imported before as it contains a side-effect required by your code.`;
}
var o = {}, s = 0;
function c(e, t, n = !1) {
	let r = function() {
		if (n && s === 0) {
			let n = `${e}.${t}`;
			o[n] || (o[n] = !0, console.warn(`[Babylon.js] ${n}() requires a side-effect import. See: https://doc.babylonjs.com/setup/treeshaking`));
		}
	};
...
var g = "attribute", _ = "varying", v = class {
	constructor() {
		this.children = [];
	}
	isValid(e) {
	...
	process(e, t, n) {
		...
		if (this.line) {
			...
			if (i) {
				...
				let r = t.processor?.attributeKeywordName ?? g, a = t.isFragment && t.processor?.varyingFragmentKeywordName ? t.processor?.varyingFragmentKeywordName : !t.isFragment && t.processor?.varyingVertexKeywordName ? t.processor?.varyingVertexKeywordName : _;
...
S.InfixToPostfixCacheLimitSize = 5e4, S.InfixToPostfixCacheCleanupSize = 25e3, S._InfixToPostfixCache = /* @__PURE__ */ new Map(), S._OperatorPriority = {
	")": 0,
	"(": 1,
	"||": 2,
	"&&": 3
...
}, D = /defined\s*?\((.+?)\)/g, O = /defined\s*?\[(.+?)\]/g, k = /#include\s?<(.+)>(\((.*)\))*(\[(.*)\])*/g, A = /__decl__/, j = /light\{X\}.(\w*)/g, M = /\{X\}/g, N = [], P = /(#ifdef)|(#else)|(#elif)|(#endif)|(#ifndef)|(#if)/;
function F(e) {
	e.processor && e.processor.initializeShaders && e.processor.initializeShaders(e.processingContext);
}
function I(e, t, n, r) {
	t.processor?.preProcessShaderCode && (e = t.processor.preProcessShaderCode(e, t.isFragment)), q(e, t, (e) => {
		t.processCodeAfterIncludes && (e = t.processCodeAfterIncludes(t.isFragment ? "fragment" : "vertex", e, t.defines)), n(K(e, t, r), e);
	});
}
function L(e, t, n) {
...
function z(e) {
	...
	if (t && t.length) return new C(t[1].trim(), e[0] === "!");
	let n = [
		"==",
		"!=",
		">=",
	...
	if (i === -1) return new C(e);
	let a = e.substring(0, i).trim(), o = e.substring(i + r.length).trim();
	return new E(a, r, o);
}
function B(e) {
	e = e.replace(D, "defined[$1]");
	let t = S.infixToPostfix(e), n = [];
	for (let e of t) if (e !== "||" && e !== "&&") n.push(e);
	else if (n.length >= 2) {
		let t = n[n.length - 1], r = n[n.length - 2];
		n.length -= 2;
		let i = e == "&&" ? new T() : new w();
		typeof t == "string" && (t = t.replace(O, "defined($1)")), typeof r == "string" && (r = r.replace(O, "defined($1)")), i.leftOperand = typeof r == "string" ? z(r) : r, i.rightOperand = typeof t == "string" ? z(t) : t, n.push(i);
	}
	let r = n[n.length - 1];
	return typeof r == "string" && (r = r.replace(O, "defined($1)")), typeof r == "string" ? z(r) : r;
}
function V(e, t) {
	let n = new x(), r = e.substring(0, t), i = e.substring(t);
	return i = i.substring(0, (i.indexOf("//") + 1 || i.length + 1) - 1).trim(), n.testExpression = r === "#ifdef" ? new C(i) : r === "#ifndef" ? new C(i, !0) : B(i), n;
}
function H(e, t, n, r) {
	let i;
	for (; U(e, n, r);) {
		...
		if (r.indexOf("#") >= 0) {
			...
			if (i && i.length) {
				switch (i[0]) {
					...
					case "#if": {
						let i = new b(), a = V(r, 3);
						t.children.push(i), i.children.push(a), H(e, i, a, n);
						break;
					}
				}
				continue;
...
function G(e, t) {
	...
	return e.processor?.shaderLanguage === 0 && (r.GL_ES = "true"), r.__VERSION__ = e.version, r[e.platformName] = "true", m(r, t?.isNDCHalfZRange, t?.useReverseDepthBuffer, t?.useExactSrgbConversions), r;
}
function K(e, t, n) {
	let r = R(e, t);
	...
	let i = t.defines, a = G(t, n);
	t.processor.preProcessor && (r = t.processor.preProcessor(r, i, a, t.isFragment, t.processingContext));
	let o = {};
	return r = W(r, a, t, o), t.processor.postProcessor && (r = t.processor.postProcessor(r, i, t.isFragment, t.processingContext, n ? { drawBuffersExtensionDisabled: !n.getCaps().drawBuffersExtension } : {}, a, o)), n?._features.needShaderCodeInlining && (r = n.inlineShaderCode(r)), r;
}
function q(e, t, n) {
	N.length = 0;
	let r;
	for (; (r = k.exec(e)) !== null;) N.push(r);
	let i = [e], a = !1;
	for (let e of N) {
		let r = e[1];
		if (r.indexOf("__decl__") !== -1 && (r = r.replace(A, ""), t.supportsUniformBuffers && (r = r.replace("Vertex", "Ubo").replace("Fragment", "Ubo")), r += "Declaration"), t.includesShadersStore[r]) {
			...
			if (e[4]) {
				...
				if (r.indexOf("..") !== -1) {
					...
					for (let e = i; e < a; e++) t.supportsUniformBuffers || (o = o.replace(j, (e, t) => t + "{X}")), n += o.replace(M, e.toString()) + "\n";
				} else t.supportsUniformBuffers || (n = n.replace(j, (e, t) => t + "{X}")), n = n.replace(M, r);
			}
			let o = [];
			for (let t of i) {
				let r = t.split(e[0]);
			...
			J.loadFile(e, (e) => {
				t.includesShadersStore[r] = e, q(i.join(""), t, n);
			});
			return;
		}
	}
	N.length = 0;
	let o = i.join("");
	a ? q(o.toString(), t, n) : n(o);
}
var J = { loadFile: (e, t, n, r, i, o) => {
	throw a("FileTools");
} }, Y = [], X = class {
...
var Q = (e, t, n, r = 16, i = 3e4, a = !0, o) => {
	if (a && Z(e, t, n)) return null;
	let s = setInterval(() => {
		Z(e, t, n) ? clearInterval(s) : (i -= r, i < 0 && (clearInterval(s), n?.(/* @__PURE__ */ Error("Operation timed out after maximum retries. " + (o || "")), !0)));
	}, r);
...
//#endregion
export { a as _, F as a, t as b, J as c, m as d, p as f, u as g, c as h, L as i, d as l, l as m, X as n, I as o, h as p, Q as r, q as s, $ as t, f as u, r as v, e as x, n as y };

//# sourceMappingURL=precisionDate-BBQlwGEL.js.map