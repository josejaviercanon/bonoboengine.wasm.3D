import { B as e, P as t, a as n, b as r, c as i, o as a, s as o, t as s, u as c, v as l, x as u, z as d } from "./Geometry-DYsNE2Rs.js";
import { d as f, t as p } from "./Filter-BsE1se_H.js";
import { t as m } from "./getPo2TextureFromSource-BRMwBqWT.js";
import { t as h } from "./canvasUtils-CPkv009i.js";
import { t as g } from "./CanvasPool-CMynuo7E.js";
...
function v(e, t) {
	...
	for (let n = 0; n < e.length; n++) {
		...
		t.matrix = i?.isCachedAsTexture ? _.copyFrom(i.textureOffsetInverseTransform).append(r.worldTransform) : i?._parentCacheAsTextureRenderGroup ? _.copyFrom(i._parentCacheAsTextureRenderGroup.inverseWorldTransform).append(r.groupTransform) : r.worldTransform, t.addBounds(r.bounds);
	}
	return t.matrix = n, t;
}
//#endregion
...
var b = class {
	...
	generateFilteredTexture({ texture: e, filters: t }) {
		...
		let r = h.getCanvasSource(e);
		if (!r) return e;
		let i = e.frame, a = e.source._resolution ?? e.source.resolution ?? 1, o = i.width, s = i.height, { canvas: c, context: l } = g.getOptimalCanvasAndContext(o, s, a);
		l.setTransform(1, 0, 0, 1, 0, 0), l.clearRect(0, 0, c.width, c.height), n.length && (l.filter = n.join(" "));
		let u = i.x * a, d = i.y * a, f = o * a, p = s * a;
		return l.drawImage(r, u, d, f, p, 0, 0, f, p), l.filter = "none", l.globalAlpha = 1, m(c, o, s, a);
	}
	_calculateFilterArea(e, t) {
		if (e.renderables ? v(e.renderables, t) : e.filterEffect.filterArea ? (t.clear(), t.addRect(e.filterEffect.filterArea), t.applyMatrix(e.container.worldTransform)) : e.container.getFastGlobalBounds(!0, t), e.container) {
	...
	_pushFilterFrame() {
		...
		return e ||= this._filterStack[this._filterStackIndex] = new b(), this._filterStackIndex++, e;
	}
	_popFilterFrame() {
...
var D = new s({
	...
	generateFilteredTexture({ texture: e, filters: t }) {
		...
		let s = i;
		this._calculateGlobalFrame(n, 0, 0, s, r.width, r.height), n.outputRenderSurface = f.getOptimalTexture(o.width, o.height, n.resolution, n.antialias), n.backTexture = u.EMPTY, n.inputTexture = e, this.renderer.renderTarget.finishRenderPass(), this._applyFiltersToTexture(n, !0);
	...
	getBackTexture(e, t, n) {
		let r = e.colorTexture.source._resolution, i = f.getOptimalTexture(t.width, t.height, r, !1), a = t.minX, o = t.minY;
	...
	_getPassthroughFilter() {
		return this._passthroughFilter ??= new T(), this._passthroughFilter;
	}
	...
	_setupFilterTextures(e, t, n, r) {
		if (e.backTexture = u.EMPTY, e.inputTexture = f.getOptimalTexture(t.width, t.height, e.resolution, e.antialias), e.blendRequired) {
	...
	_updateFilterUniforms(e, t, n, r, i, a, o, s) {
		let c = this._filterGlobalUniforms.uniforms, l = c.uOutputFrame, d = c.uInputSize, f = c.uInputPixel, p = c.uInputClamp, m = c.uGlobalFrame, h = c.uOutputTexture;
		o ? (l[0] = n.bounds.minX - r, l[1] = n.bounds.minY - i) : (l[0] = 0, l[1] = 0), l[2] = e.frame.width, l[3] = e.frame.height, d[0] = e.source.width, d[1] = e.source.height, d[2] = 1 / d[0], d[3] = 1 / d[1], f[0] = e.source.pixelWidth, f[1] = e.source.pixelHeight, f[2] = 1 / f[0], f[3] = 1 / f[1], p[0] = .5 * f[2], p[1] = .5 * f[3], p[2] = e.frame.width * d[2] - .5 * f[2], p[3] = e.frame.height * d[3] - .5 * f[3];
		let g = this.renderer.renderTarget.rootRenderTarget.colorTexture;
		m[0] = r * a, m[1] = i * a, m[2] = g.source.width * a, m[3] = g.source.height * a, t instanceof u && (t.source.resource = null);
		let _ = this.renderer.renderTarget.getRenderTarget(t);
		this.renderer.renderTarget.bind(t, !!s), t instanceof u ? (h[0] = t.frame.width, h[1] = t.frame.height) : (h[0] = _.width, h[1] = _.height), h[2] = _.isRoot ? -1 : 1, this._filterGlobalUniforms.update();
	}
	...
	_applyFiltersToTexture(e, t) {
		...
		if (this._globalFilterBindGroup.setResource(n.source.style, 2), this._globalFilterBindGroup.setResource(e.backTexture.source, 3), a === o) i[a].apply(this, n, e.outputRenderSurface, t);
			...
			let n = e.inputTexture, s = f.getOptimalTexture(r.width, r.height, n.source._resolution, !1), c = s;
			for (let e = a; e < o; e++) {
				let t = i[e];
			...
			i[o].apply(this, n, e.outputRenderSurface, t), f.returnTexture(s);
		}
	}
	_calculateFilterBounds(e, t, n, r, i) {
		let a = this.renderer, o = e.bounds, s = e.filters, c = Infinity, u = 0, d = !0, f = !1, p = !1, m = !0, h = -1, g = -1;
		for (let e = 0; e < s.length; e++) {
			let t = s[e];
			if (t.enabled) {
				if (h === -1 && (h = e), g = e, c = Math.min(c, t.resolution === "inherit" ? r : t.resolution), u += t.padding, t.antialias === "off" ? d = !1 : t.antialias === "inherit" && (d &&= n), t.clipToViewport || (m = !1), !(t.compatibleRenderers & a.type)) {
					p = !1;
					break;
				}
				if (t.blendRequired && !(a.backBuffer?.useBackBuffer ?? !0)) {
					l("Blend filter requires backBuffer on WebGL renderer to be enabled. Set `useBackBuffer: true` in the renderer options."), p = !1;
					break;
				}
		...
		if (m && o.fitBounds(0, t.width / r, 0, t.height / r), o.scale(c).ceil().scale(1 / c).pad((u | 0) * i), !o.isPositive) {
			e.skip = !0;
			return;
		}
		e.antialias = d, e.resolution = c, e.blendRequired = f, e.firstEnabledIndex = h, e.lastEnabledIndex = g;
	}
	_popFilterData() {
	...
	_pushFilterData() {
		let e = this._filterStack[this._filterStackIndex];
		return e ||= this._filterStack[this._filterStackIndex] = new O(), this._filterStackIndex++, e;
	}
};
k.extension = {
...
//# sourceMappingURL=init-Ts9xUJLu.js.map