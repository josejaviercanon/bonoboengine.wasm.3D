import { _ as e, t, u as n } from "./math.color.pure-DY3Xz9Wt.js";
import { t as r } from "./math.scalar.functions-DcC_Ew_o.js";
import { n as i, t as a } from "./sphericalPolynomial.pure-BcS1xpZz.js";
...
var o = class {
	...
	static ConvertCubeMapToSphericalPolynomial(n, o = 0) {
		let s = o > 0 ? this._NearestPow2Floor(o) : 0;
		if (s > 0 && n.size > s) {
			...
			for (let i of t) r[i] = this._DownsampleFace(n[i], n.size, s, e);
			n = {
				...n,
				...r,
				size: s
		...
		let c = new a(), l = 0, u = 2 / n.size, d = u, f = .5 * u, p = f - 1;
		for (let i = 0; i < 6; i++) {
			let a = this._FileFaces[i], o = n[a.name], s = p, m = n.format === 5 ? 4 : 3;
			for (let i = 0; i < n.size; i++) {
				let h = p;
				for (let d = 0; d < n.size; d++) {
					let p = a.worldAxisForFileX.scale(h).add(a.worldAxisForFileY.scale(s)).add(a.worldAxisForNormal);
					p.normalize();
					let g = this._AreaElement(h - f, s - f) - this._AreaElement(h - f, s + f) - this._AreaElement(h + f, s - f) + this._AreaElement(h + f, s + f), _ = o[i * n.size * m + d * m + 0], v = o[i * n.size * m + d * m + 1], y = o[i * n.size * m + d * m + 2];
					isNaN(_) && (_ = 0), isNaN(v) && (v = 0), isNaN(y) && (y = 0), n.type === 0 && (_ /= 255, v /= 255, y /= 255), n.gammaSpace && (_ = r(_) ** +e, v = r(v) ** +e, y = r(y) ** +e);
					...
					} else _ = r(_, 0, b), v = r(v, 0, b), y = r(y, 0, b);
					let x = new t(_, v, y);
					c.addLight(p, x, g), l += g, h += u;
				}
				s += d;
			}
		}
		let m = 4 * Math.PI * 6 / 6 / l;
		return c.scaleInPlace(m), c.convertIncidentRadianceToIrradiance(), c.convertIrradianceToLambertianRadiance(), i.FromHarmonics(c);
	}
};
s._FileFaces = [
	new o("right", new n(1, 0, 0), new n(0, 0, -1), new n(0, -1, 0)),
	new o("left", new n(-1, 0, 0), new n(0, 0, 1), new n(0, -1, 0)),
	new o("up", new n(0, 1, 0), new n(1, 0, 0), new n(0, 0, 1)),
	new o("down", new n(0, -1, 0), new n(1, 0, 0), new n(0, 0, -1)),
	new o("front", new n(0, 0, 1), new n(1, 0, 0), new n(0, -1, 0)),
	new o("back", new n(0, 0, -1), new n(-1, 0, 0), new n(0, -1, 0))
], s.MAX_HDRI_VALUE = 4096, s.PRESERVE_CLAMPED_COLORS = !1;
//#endregion
export { s as t };

//# sourceMappingURL=cubemapToSphericalPolynomial-BrdoDd_Y.js.map