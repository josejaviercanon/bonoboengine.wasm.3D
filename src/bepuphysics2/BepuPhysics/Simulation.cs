...
public partial class Simulation : IDisposable
{
    ...
    /// </summary>
    /// <remarks>
    /// While <see cref="BepuPhysics.Collidables.Shapes"/> instances can be shared between multiple <see cref="Simulation"/> instances, there are no guarantees of thread safety.
    /// Further, setting the <see cref="Shapes"/> property to another collection while there are any outstanding references in the simulation to shapes within the old collection will result in fatal errors if the simulation continues to be used.
    /// </remarks>
    ...
    /// <param name="shapes">Collection of shapes to use in the simulation, if any. If null, a new <see cref="BepuPhysics.Collidables.Shapes"/> collection will be created for this simulation.</param>
    /// <returns>New simulation.</returns>
    public static Simulation Create<TNarrowPhaseCallbacks, TPoseIntegratorCallbacks>(
        BufferPool bufferPool, TNarrowPhaseCallbacks narrowPhaseCallbacks, TPoseIntegratorCallbacks poseIntegratorCallbacks, SolveDescription solveDescription, ITimestepper timestepper = null, SimulationAllocationSizes? initialAllocationSizes = null, Shapes shapes = null)
        ...
        where TPoseIntegratorCallbacks : struct, IPoseIntegratorCallbacks
    {
        ...
        simulation.Shapes = shapes == null ? new Shapes(bufferPool, initialAllocationSizes.Value.ShapesPerType) : shapes;
