...
// All pixel values scale relative to this (pointer inputs for sliders are fully adjusted for adaptive scaling in 9.x)
...
The Babylon.js GUI API remains backwards compatible in the 9.x release cycle. Key engine updates in this release include:

* **Adaptive Scaling Fixes:** Slider UI pointer inputs are now accurately adjusted for adaptive scaling across varying screen sizes.
* **Internal Migrations:** The Babylon GUI package has migrated its test suites from Jest to Vitest and dropped unused NPM dependencies.
* **Signed Distance Field Text:** While general GUI `TextBlock` functions remain the same, 9.x introduces Signed Distance Field Text support which greatly improves the rendering crispness of in-scene 3D text (distinct from the 2D GUI module).
