...
For **Babylon.js 9.x**, several core additions, modern capabilities, and architectural updates (such as advanced Gaussian Splatting support, Outline Renderer updates, and new performance features) should be integrated.
...
- [Gaussian Splatting & Advanced Meshes](#gaussian-splatting--advanced-meshes)
...
For composing many primitives into a reusable **parametric template** (a shuttle, a chassis, a lipped post — anything that varies by size and repeats hundreds to millions of times), see **[parametric-factories.md](https://www.google.com/search?q=parametric-factories.md)**. It covers PROPORTIONS tables, `Dims`, shared material factories, `ExtrudePolygon` for non-box cross-sections, template caching, and thin-instance templates.

## Gaussian Splatting & Advanced Meshes

Babylon.js 9.x introduces comprehensive support for advanced radiance fields and specialized mesh extensions:

* **Multi-Format Support & Compression:** Load `.ply`, `.splat`, `.spz`, and Fraunhofer HHI’s compressed `.sog` / `.sogs` formats seamlessly.
* **Gaussian Splat Shadow Casting:** Splats can now cast contact-hardening shadows when transparency shadow support is active on the shadow generator.
* **Triangle Splatting:** Support for opaque, vertex-colored triangular geometry options for radiance fields to integrate seamlessly into complex opaque scenes.
* **Scene Composition:** Combine multiple splat assets into a single scene with global sorting, independent child transforms, and GPU-based picking.
...
// Enable edges (enhanced via the 9.0 Outline Renderer and edge updates)
