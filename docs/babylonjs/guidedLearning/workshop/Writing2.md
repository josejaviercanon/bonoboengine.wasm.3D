...
image:
description: Using a shader to produce a firework that bursts
...
The requirements are for a collection of colored facets to burst out from a central position so that each fragment lies on the surface of an expanding sphere.
...
are accessible to the shader as attributes, so passing time as a uniform will be enough.

To obtain an initially fast expansion that then slows down, a logarithmic function of time will be used.

## Second Stage Design

The color and transparency of all the facets will be the same at the same time, so a change to each component of the color of a vertex
can be applied as a function of time.

## Third Stage Design

After the first coding of the shaders, adjustments were made manually to the time functions to obtain a satisfactory appearance for the burst of facets.
