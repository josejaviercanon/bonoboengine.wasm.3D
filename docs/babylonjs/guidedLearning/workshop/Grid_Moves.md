...
Other systems are possible that fit the above conditions except for the collision rules.
...
particle. The position of any particle will be the centre of the cube it is occupying. The conditions on the particles' speeds mean that each one will lie wholly within a cube.
...
Now that we have constrained the positions of the walls to ensure that the particles rebound on just touching them, can the same be done for the particles themselves?
...
Systems in which the Newtonian laws of physics are obeyed are considered [TBA]. This page now considers other physics.
...
What about collisions between particles in these cases? To ensure contact by touching alone, we have already seen that there must be an even number of cubes between them. Placing particles in the box so that this condition exists is not straightforward, so we simplify it to this: every particle has two cubes between it and the next particle. This means that, placing a particle at the cube origin and counting cube positions from this point, every cube with a position of the form (3m, 3n, 3p), and only these cube positions, has a particle in it. Should one particle be missing, there will be two particles with an odd number of cubes between them, which could meet by overlapping, as in Fig 13.

![Fig 13](/img/samples/collide12.webp)  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Fig 13

However, setting random velocities for such particles could lead to a situation such as in Fig 14, which would again cause particles to overlap completely.
...
A second condition is that the particles must be travelling towards each other. By considering relative velocities, one particle can be considered still.
The component of velocity for the moving particle along the line joining their centres will be positive or negative depending on whether it is moving away from or toward the other. The magnitude of the component velocity is found by using the dot product of their relative positions and relative velocities.
...
In both these examples, one particle is colored red to aid focus on a single particle if needed. Also, by removing the comment markers on lines 140 and 144 in the swap example, and lines 137 and 141 in the reverse example, you can see the path trace of the red particle.
