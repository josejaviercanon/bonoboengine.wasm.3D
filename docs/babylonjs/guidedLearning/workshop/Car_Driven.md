...
the car. This view will be fixed because the camera will not be attached to the canvas. The front wheels will be further away from the side of the body
to allow them to turn, and each will have its own pivot. The car will be driven forward using the space bar as an accelerator up to a maximum speed.
The front wheels will be steered with A and D. The rotation of the wheels will be matched to the speed of the car.
...
line through the pivot point normal to the wheel meets the line through the two rear wheel supports.
...
of the two rear wheels it is A/2 + L/tan(theta).

## Mathematics of Rotation for Car

When the Babylon.js engine is running at F frames per second, the time for one frame is 1/F seconds.

When the speed of the car is D units per second the distance travelled by the car is D/F units.

When the car is driven forward (i.e. theta = 0) at a speed of D units per second, the distance travelled by the car in one frame is D/F units.
...
When the car is turning, each wheel will rotate at different rates because they travel different distances in the same time. For simplicity, the following
...
The rear inside wheel travels a distance R * phi = D/F and so phi = D/RF.
...
R - NR along the local positive z axis, while the car is translated NR - R along the local positive z axis. This results in the car not changing position
...
The second playground is similar. The pivot has been made into a sphere so you can see it, and the camera position has been changed to give
