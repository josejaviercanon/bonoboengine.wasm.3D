...
Making our game cross-platform is really simple with Babylon.js!
...
Now that we're no longer using key presses for our character, we need another way to get them moving around. This is where it gets a little tricky. The joystick class works really well if the game's main mechanic is using only the joystick. The joystick class creates an overlay canvas that ends up blocking any other inputs that may be on the screen. Since we need buttons to also jump and dash, I had to find a different way to get player movement. The final layout looked like this:

![mobile controls](/img/how_to/create-a-game/mobilecontrols.webp)
...
When you make a grid, you add columns separately and pass in a size for each one.
...
If you look at the code above, you can actually see the grid placement of the buttons based on the location of 0s and 1s (of course, this only works in this case since there are just two buttons). This is how you get the effect of them being stacked on opposite sides of each other!

![grid](/img/how_to/create-a-game/gridbuttons.webp)

## Movement

Now that we've made the new GUI elements for our character movement, we just need to hook them up to our current input system.
...
Then all we need to do is add the mobile equivalent of our key presses to the existing structure in \_updateFromKeyboard. For example, dashing would be:
...
![mobile gameplay](/img/how_to/create-a-game/mobilegameplay.webp)

Now we have a cross-platform game! Wasn't that easy?

## Extra Features

Because the game is in a web browser, we need to make sure that the player knows the intended orientation is landscape mode. Not all browsers or devices allow fullscreen mode, so I decided to create a notification that told the player to rotate the device to get the best experience.
...
![image of notification](/img/how_to/create-a-game/rotatedevice.webp)
