...
keywords: diving deeper, physics, soft bodies
further-reading:
...
In terms of physics engines, a soft body is a body whose shape can be deformed by interactions with other bodies or forces within the physics world. Of the three plugins available with Babylon.js, soft bodies are available only with Ammo.js. There are three soft bodies you can create: the three-dimensional `softbody`, the two-dimensional `cloth`, and the one-dimensional rope. First, a number of limitations need to be considered, followed by some aspects particular to soft bodies and then some examples.
...
A deformation of a body can only be shown if there are enough vertices to produce the deformation. The number of vertices used, along with the need to check their positions and velocities at each time step, means that the number of soft bodies may be limited to one, two, or possibly three.
...
One benefit of this is that it reduces the number of vertices in the mesh. Of course, there may still not be enough vertices in the mesh to make a properly functioning soft body. In that case, use

```javascript
mesh.increaseVertices(n);
```

where n is the number of extra vertices to add to each side of a triangular facet. The number of facets is increased by a factor of (n + 1)<sup>2</sup>. This method forces repeated vertex positions to share indices. The order for using the two methods above is always share first, then increase.
...
3. Soft bodies cannot have forces and impulses applied to them.

## Options for Soft Bodies

As well as the usual optional parameters of mass, friction, and restitution that can be set when creating a physics impostor, there are a number of parameters specific to soft bodies. These come in two sets.

Those that can be set during or after impostor creation:
...
- stiffness, a float from 0 to 1: 0 is very elastic, so the distance between vertices is not constrained; 1 is inelastic, so the distance between vertices is invariant; default 1;
...
- margin, the collision margin around the soft body; when needed, set it to around 1/100 of the body's size. It is particularly useful for stopping a cloth from passing through a body; default 0;
...
You may have to use trial and error with the above parameters to get the effect you want.
...
It also has to have enough vertices to simulate the body. When the mesh used cannot [increase the number of vertices](/toolsAndResources/utilities/Increasing_Facets) during its construction, a box for example, you use
...
A 2D soft body. All cloth soft bodies are created from a [ground mesh](/features/featuresDeepDive/mesh/creation/set/ground), as this is already two-dimensional, has shared vertices, and can have its number of vertices increased by changing the subdivisions used. The usual [options](/legacy/physics/softBodies#options-for-soft-bodies) are available.
...
Setting an anchor requires three parameters plus two optional ones, as in
...
- fraction of width, from 0 to 1, the fraction across the width of the ground mesh
- fraction of height, from 0 to 1, the fraction up the height of the ground mesh
- influence, from 0 to 1, defaults to 1. A value of 1 fixes the distance of the anchor from the rigid body; the closer it is to 0, the more elastic the distance
- noCollisionBetweenLinkedBodies, default false. When true, the soft body and the rigid body to which it is attached will not detect collisions with each other.
...
A rope is a 1D soft body. It can be constructed using a [lines mesh](/features/featuresDeepDive/mesh/creation/param/lines) or an [extruded shape](/features/featuresDeepDive/mesh/creation/param/extrude_shape) mesh. The usual [options](/legacy/physics/softBodies#options-for-soft-bodies) are available for both; however, using an extruded shape requires extra parameters in the options.

### Using Lines

A lines mesh should be constructed in a straight line, and an impostor should be constructed in the same way as for a softbody or a cloth.
...
In the following Playground, remember that the extruded shape does not have an impostor. It lies on the surface of the box because the margin for the rope is set to the radius of the extruded shape.
...
A hook can be added anywhere along the length of the rope using a number from 0 (start) to 1 (end) and attached to a rigid body using
...
- rigidImpostor, to which the hook is attached. The attachments are calculated automatically by Ammo.js and depend on the position of the ropeImpostor and the hook used.
- fraction of length, from 0 to 1, the fraction across the length of the rope
- influence, from 0 to 1, defaults to 1. A value of 1 fixes the distance of the hook from the rigid body; the closer it is to 0, the more elastic the distance
- noCollisionBetweenLinkedBodies, default false. When true, the rope and the rigid body to which it is attached will not detect collisions with each other.

For example, you can attach a ball to the end of a rope and a crossbar to the start of a rope with influence 1:
...
<Playground id="#8WC6ZN#4" title="Swinging Ball (Lines Mesh)" description="Simple example of a swinging ball using a lines mesh."/>
