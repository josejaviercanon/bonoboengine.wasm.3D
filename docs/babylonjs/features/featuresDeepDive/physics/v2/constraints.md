...
```javascript
// This will constrain the bodies to maintain a distance of at least 1 and at most 2, and to rotate at most 1.58 rad along the perpendicular axis
...
The physics viewer allows you to display the angular and linear limits of constraints.
The following example adds and immediately removes a constraint from debug visualization.
...
For performance reasons, the limit debug mesh is not recreated when constraints change. The user is responsible for removing and re-adding the constraint if any limit is changed.
...
When simulating a long chain of constrained bodies, not all of the constraints might be solved perfectly by the physics engine, which can result in stretching or misalignment of the constraint. This is more likely to happen if the mass ratio between two constrained bodies is very high. A common way to mitigate this is to tweak the mass ratios of the constrained bodies: increase the mass and inertia of bodies that are closer to the "root" of the chain, and decrease them as the bodies get farther from the root. For example, if you have a ragdoll, ensure that the torso has the most mass and decrease the mass as you move down the limbs.
