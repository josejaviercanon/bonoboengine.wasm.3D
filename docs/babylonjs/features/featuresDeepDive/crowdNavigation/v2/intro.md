...
V1 uses an old and unmaintained JavaScript package, which often causes issues when used with ES6 and bundlers. V1 is being deprecated.

V2 uses the remarkable [recast-navigation-js](https://github.com/isaac-mason/recast-navigation-js) library by Isaac Mason (thank you Isaac!), which is fully ES6-compliant, up to date, and well maintained.

V2 will eventually replace V1 after it gets thoroughly tested. That is why V2 is in `ADDONS` and not already in the `core` package. Since the two versions should be fully compatible and only the internal implementation details and the underlying Recast/Detour libraries have changed, you can easily switch to V2 without rewriting your code.

There is one change, though. You need to initialize the plugin by calling an `async` factory function instead of using a constructor, as in V1.
...
This call will dynamically load the `recast-navigation-js` library and initialize it.
...
Most of the functions in the navigation plugin interface are self-explanatory, even for first-time users. It is recommended to get familiar with concepts at Recast: [https://recastnav.com/](https://recastnav.com/).

Since the navigation plugin V2 acts as a facade around `recast-navigation-js`, it is recommended to visit the recast-navigation-js [GitHub repo](https://github.com/isaac-mason/recast-navigation-js) as well.
