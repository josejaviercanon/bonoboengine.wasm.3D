...
There are also three properties you can read:
...
Please also note that even when a particle is invisible (_isVisible_ set to _false_), its other property values can be updated, and _updateParticle()_ is called for every particle whether it is visible or not.
...
In the simple one-dimensional case, let a particle be at `positionX` with a pivot at `pivotX`. The center of transformation depends on whether _translateFromPivot_ is true. When false, it is at the point `positionX + pivotX`, and when true, it is at the point `positionX`. The distance of the point from the pivot is then `pivotX`.
...
Illustration of a pivot particle with 1D pivot placement; change the data on lines 23 - 25 to see the effect.

<Playground id="#LXXL6Y#1" title="Scaled Pivot Example" description="Simple example of scaling a pivot on a solid particle"/>

Pivot particle set in 3D, with rotation and scaling applied - 1000 tetrahedron satellites orbiting around 1000 rotating boxes.

### Billboard

When you set your SPS in billboard mode, you should only set a _rotation.z_ value. Setting _SPS.billboard = true_ makes all the particles face the camera, and their _x_ and _y_ rotation values will be ignored. This is useful if you display only plane particles. However, if you work only with 2D particles, you should consider using the particle system or one of the various sprite managers, which are more performant for 2D computation. To display the SPS in billboard mode, you need to call _SPS.setParticles()_ within the scene render loop.
...
When you set a particle color, all vertices of the particle are set to that color and will blend nicely with any material colors, if applied.

On creation, the vertex colors of the particle are the vertex colors of the added model. When the model has vertices that use a range of colors, a box with different face colors, for example, these colors are preserved by the particle. However, setting the color of a particle results in a single color for that particle. When the model mesh has no colors, the particle color is set to white.
...
Note: There is another way to set the initial state of the particles, which you will learn about on the next page about immutable SPS. This is done by using the _positionFunction_ option property when adding a shape. When this method is used, particle colors are null, so a new Color4 object must be created to set _particle.color_, following this form:

```javascript
particle.color = new BABYLON.Color4(0.25, 0.333, 0.67, 0.88);
```

When you want to set an alpha value, do not forget to enable the alpha channel for vertex colors:
...
On the next page, we will look at how SPS can be configured to use multi-materials. For now, we consider the use of one material with one texture. The image file you use should contain all the textures you want to apply to the particles as a texture atlas.

To apply a texture from a region of the image file, you specify its lower-left (x, y) and upper-right (z, w) coordinates as fractions of the whole image in the form of a UV `Vector4` (x, y, z, w). Taking the lower-left coordinates as (0, 0) and the upper left as (1, 1) will apply the whole image as a material. The _uvs_ property of a particle is a `Vector4`, and for the whole image you would use a UV of (0, 0, 1, 1).
...
This can be used by either of the custom functions, _positionFunction_ or _updateParticle()_, whichever you are using. It is most useful when the image is a texture atlas.
...
When the model mesh you are adding to the SPS uses the [material per face technique](/features/featuresDeepDive/materials/using/texturePerBoxFace), the correct uv values are applied automatically when the model is added as a shape. Unlike colors, the model UVs are preserved whether or not the model had per-face UVs. This lets you use not only a texture atlas for the particles, but also a texture atlas for the model within the particle atlas, because you use only one final texture.
...
The model is a box with per-face material. Scaling the FaceUVs by 2 groups the 24 individual characters into 6 groups of 4. The particles' materials are direct copies of those of the model.
...
The model is a box with per-face material applied as before, with a group of four per face. An area of the texture for each face is then chosen using the particles' _uvs_, in this case by halving both the width and height for all particles as in the image below.

![Sprite Altlas](/img/how_to/apply-material-to-faces/spriteAtlas3.webp)

Transparency

Like any other mesh, you can also enable texture transparency with:
...
When a particle has a parent, its position and rotation are then expressed in its parent's local space.

In the following code, the previous particle becomes the parent of the current one. The particle position and rotation are expressed in the previous particle's space, which is already rotated and translated from the particle before it, and so on.
...
A third box is added to the last two, rotating about the previous one.
...
Using TypeScript (or JavaScript), you can associate your own properties with each particle through the _props_ property, typed _any_ and set to _null_ by default:
...
To manage particle behavior, you use the mandatory method _setParticles_ along with some non-mandatory custom functions. The _setParticles_ method is necessary to update and display the mesh built by the SPS.

The non-mandatory custom functions are:
...
The following pseudo-code shows how they are applied.
