...
Previously, we saw how to use an SPS to optimize a scene containing multiple copies of a model mesh, provided the model properties remained fixed. Now we will look at ways to optimize when the particles are animated.
...
For any solid particle animation, *setParticles()* is called on every render frame. When you have a large SPS with dozens of thousands of particles, you can improve performance by restricting the number of particles updated on each frame. There are three parameters you can use with *setParticles()* to do this. They are:
...
If you pass a *stop* value greater than *nbParticles - 1*, the iteration will stop at *nbParticles - 1* to prevent access to undefined elements.
...
Example 2: you could keep, say, the first 5,000 particles as unused ones and compute particle behavior only for the last 5,000 in your global pool.

### Limit Garbage Collection

In JavaScript, the Garbage Collector is usually your friend: it takes care of cleaning up variables you no longer need and thus frees memory.  
However, it can sometimes become an awkward friend because it may start cleaning while you are displaying a very smooth animation, taking the CPU for itself and leaving you with visible lag on the screen.  
To avoid unpredictable GC pauses, it is best to avoid allocating new objects in loops that execute often, where particles are created or updated.  
...
Here is an implementation of a simple particle AI called "flocking", a behavior based on association, cohesion, and separation. This example uses *SPS.vars* to allocate the memory used for results only once instead of using in-function temporary variables.  
