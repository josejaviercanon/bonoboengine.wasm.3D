...
New properties can be initialized.
...
When using `addPoints`, particle properties can be set in the passed function. The `addSurfacePoints` and `addVolumePoints` methods obviously set the position and color properties of the particles, but you may still want to set the initial values of other particle properties.

This can be done using `initParticles`. To do this, iterate over all the particles by using the `nbParticles` property, and follow the call to this function with a call to `setParticles()`. For example:
...
Once the appropriate particle properties are initialized, the `updateParticles` method can be used. Unlike `initParticles`, this function is called by `setParticles` and already receives a particle as an argument. The `setParticles` method will execute only after the PCS mesh has been built, so it can safely be placed inside a render loop to produce an animation. For example:
...
The particle `pivot` vector is also in the *local space* of the PCS mesh. By default, rotations around a pivot are calculated by translating the particle to the pivot point, then rotating it, and then applying the inverse translation. By setting the particle property `translateFromPivot` to `true` (default `false`), rotations are calculated using only the initial translation followed by the rotation, leaving the particle at the translated location.  

<Playground id="#UI95UC#12" title="Simple Animation" description="Simple example of animating a point cloud system."/>

In the following playground, the particle pivots in the top PCS are set relative to the particle position, and in the lower one they are set in the same place.
...
This playground loads meshes from a file, converts them to particles, and animates them.
...
While setting and changing particle colors is straightforward, doing the same for UVs is a little more complex. Using `BABYLON.PointColor.UV` as a parameter within `addSurfacepoints` or `addVolumePoints` will set the UVs automatically based on the passed mesh.

You can also set the UV value for each particle using the function passed to `addPoints`. To make the texture map correctly, the UV values should relate in some way to the positional values for each particle. To apply the texture with these UVs, both the `emissiveColor` and `emissiveTexture` for the PCS mesh material must be set after the mesh is built.
...
The parent must be created before the child particle. This means the parent must have a lower index ID (`particle.idx`) than the current particle. So the first particle in the pool (`idx = 0`) cannot have a parent. To give a parent to a particle, just set its property `parentId` to the parent index ID value. 
...
To use it, call the method `intersectsMesh(target)` (`target` is a mesh) for any particle to check whether it intersects the _target_.  
It will simply return `true` or `false` depending on whether the particle intersects the target.    
...
```
By default, the check is carried out on the mesh's (AABB) bounding box. If you want to use the mesh's bounding sphere, add the parameter `true`.
```javascript
