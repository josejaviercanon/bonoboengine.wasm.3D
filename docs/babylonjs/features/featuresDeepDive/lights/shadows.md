...
In this tutorial, we are going to learn how to create shadows in Babylon.js. Shadows are now dynamic, and they are generated depending on a light.
...
And finally, you will have to define where the shadows will be displayed by setting a mesh property to true:
...
If you want to go further, you can activate shadow filtering to create better-looking shadows by removing the hard edges.
...
It is `true` by default because it is useful for reducing shadow aliasing. But if you want to reduce computation time, feel free to turn it off.
...
This is the softest shadow filter, but it is also the slowest. It uses a blurred exponential shadow map.

The quality of the blur is defined by the following properties:

- `shadowGenerator.blurScale`: Defines the scale used to downscale the shadow map before applying the blur postprocess. By default, the value is 2.
- `shadowGenerator.blurBoxOffset`: Defines the offset of the box's edge used to apply the blur. By default, the value is 1 (meaning the box will go from -1 to 1 in both directions, resulting in 9 values read by the blur postprocess).
- `shadowGenerator.useKernelBlur`: You can decide to use kernel blur instead of box blur. While a bit more expensive, shadow quality is far better with kernel blur. You can control the kernel size with `shadowGenerator.blurKernel`, whose default value is 1.
...
### Percentage Closer Filtering (WebGL 2 only)
...
PCF shadows benefit from the new hardware filtering functions available in WebGL 2 and produce a smoother version of Poisson sampling. They fall back to standard Poisson sampling when WebGL 2 is not available on the target device.
...
As PCF requires more resources than may be available on small platforms, you can use the `filteringQuality` property to choose the best tradeoff between quality and performance depending on your needs (the lower the quality, the better the performance).

```javascript
shadowGenerator.filteringQuality = BABYLON.ShadowGenerator.QUALITY_LOW;
```

### Contact hardening shadow (WebGL 2 only)

Starting with Babylon.js 3.2, contact-hardening shadows based on PCSS were introduced.

PCSS can be seen as an improved version of PCF, but despite looking better, they are also more processor-intensive and should be reserved for desktop applications. Like PCF, they automatically fall back to Poisson sampling if the code is running on a WebGL 1 platform.
...
In order to get accurate results, you will need to define additional parameters:
...
As PCSS requires more resources than may be available on small platforms, you can use the `filteringQuality` property to choose the best tradeoff between quality and performance depending on your needs (the lower the quality, the better the performance).
...
Starting with Babylon.js v4.2, you can simulate soft transparent shadows for transparent objects. To do this, you need to set the `enableSoftTransparentShadow` property to `true` on the shadow generator:
...
It works by generating dithering patterns in the shadow map based on the alpha value of the fragment. This pattern can be visible depending on your objects, their zoom level, and/or the filtering method used.

For example, here's what it looks like when you don't set a filtering method:
...
Even when using a filtering method, you may still be able to see the patterns if you zoom in too much (filtering=PCF):
...
This one was generated with PCF, for example.
...
Keep in mind that this shadow generator can only be used with one light. If you want to generate shadows from another light, you will need to create another shadow generator.

Only point, directional and spot lights can cast shadows.

### Point lights

Point lights use cubemap rendering, so please be cautious when enabling them, as this could lead to some performance issues.
...
To optimize rendering, you can also decide to use the point light like an unlimited spotlight if you are sure that all shadow casters are on the same side of the light. To do so, just specify a direction for your light, and Babylon.js will automatically use a simple texture for the shadow map instead of the cubemap.
...
Directional lights use orthogonal projection. The light's position is evaluated automatically for you to get the best shadow map possible. You can control this behavior by turning `light.autoUpdateExtends` off.
You can also control the size of the projection window by modifying one of these properties:

- `light.shadowOrthoScale`: 0.1 by default, which means that the projection window is increased by 10% from the optimal size.
- `light.shadowFrustumSize`: Off by default with a value of 0. You can specify a value that will be used to define the square size of the frustum to use.

The light's position, as well as the positions of the meshes that you have pushed into the renderList, determine where the shadows will appear. Note that the light's point of view from this position has to include all meshes in the renderList; otherwise, the shadows may not be rendered. See the following example: <Playground id="#R1EVD0#152" title="Animated Directional Light Example" description="Simple Example of a moving directional light." image="/img/playgroundsAndNMEs/divingDeeperShadows10.webp" isMain={true} category="Shadows"/>

You can also set `light.autoCalcShadowZBounds = true` to automatically compute the best `light.shadowMinZ` and `light.shadowMaxZ` values for each frame. Tightening those values to best fit your scene improves the precision of the depth map, and consequently the shadow rendering. Be warned, however, that when using this parameter with PCF and PCSS, you may miss some shadows because of the way those filtering techniques are implemented (if all your meshes are both shadow casters and receivers, you won't have any problems with PCF / PCSS, though). Note that `light.autoUpdateExtends` must be set to `true` for `light.autoCalcShadowZBounds` to work.
...
#### In-depth analysis of directional light position

The light position is set to `-light.direction` at creation time, and you can change it later by accessing `light.position`.
...
Even if you end up using an orthographic projection (as you do for directional lights), you still need a view matrix to transform the geometry into the light coordinate system (where the "look at" vector corresponds to the light direction for directional lights). To build this matrix, you **really** need a position!

Often, you can read explanations about directional lights that say you don't need a position, but if you look at their code, you can see they use a `(0,0,0)` position to build their view matrix... Or, in others, they don't explicitly put values in the translation part of the matrix, but you still get 0 values there by default.

This position defines where the light frustum is positioned in the world. See the following example: <Playground id="#2898XM#3" title="Analyzing Directional Light Position" description="Simple Example of analyzing directional light position." image="/img/playgroundsAndNMEs/divingDeeperShadows10.webp"/>
...
It's not very obvious in the screenshot, but the light position lies on the near plane. This is because we set `light.shadowMinZ = 0` (we have also set `light.shadowMaxZ = 3`, which is the distance to the far plane of the light frustum). If we change the value, the near plane (and the light frustum) will be moved accordingly, while the light position remains the same:
...
According to the explanations above, the points on the ground that are not inside the cube should not be shadowed! They still are because the shadowing code does not apply rejection based on depth, only on the x/y coordinates: if the point is inside the frustum according to the left/right/top/bottom planes, it's okay, even if the point is farther than the far plane (or nearer than the near plane).
...
So at this point, you may ask: why not set `shadowMinZ` to a very small value (-1e10) and `shadowMaxZ` to a very large value (1e10) to get rid of those problems? One could even set the left/right/top/bottom properties to very small/large values and call it a day: no more problems with directional lights, because the frustum would always be big enough to contain all the objects in the scene.
...
Shadow mapping is a great technique, but it is not perfect. Several parameters can be tweaked to help improve the final rendering.

### Bias

You may want to reduce shadow acne resulting from an imprecise shadow map. To do so, you can define the bias (which is 0.00005 by default):
...
As mentioned earlier, if you want blurred shadows on a self-shadowing object, the best option will probably be to use close exponential shadow map.

### Frustum edge falloff

Depending on how you set up your shadow generator, you could see strange falloff when an object is near the edges of the shadow map. To fix this issue elegantly, you can set a property named `frustumEdgeFalloff`:
...
Self-shadowing probably requires the most attention during setup. Let's try to set up self-shadowing on the following scene: <Playground id="#FH3FM2#77" title="Self-Shadowing Step 1" description="Simple Example of self shadowing in your scene." image="/img/playgroundsAndNMEs/divingDeeperShadows13.webp"/>

The first step consists of adding a shadow generator to the scene and defining every mesh as both a caster and a receiver (we also force the bias to 0 to highlight the generated artifacts): <Playground id="#FH3FM2#78" title="Self-Shadowing Step 2" description="Simple Example of self shadowing in your scene." image="/img/playgroundsAndNMEs/divingDeeperShadows14.webp"/>

As you can notice, there are strange patterns appearing everywhere on the surface of the self-shadowed objects. This is called shadow acne ([more information](http://www.opengl-tutorial.org/intermediate-tutorials/tutorial-16-shadow-mapping/#shadow-acne)).

Fortunately, in Babylon we do have a way to solve the issue.

#### Bias

As detailed in the previous [opengl tutorial](http://www.opengl-tutorial.org/intermediate-tutorials/tutorial-16-shadow-mapping/#shadow-acne), you can increase the value of the bias to make all the acne disappear: <Playground id="#FH3FM2#79" title="Self-Shadowing Step 3" description="Simple Example of self shadowing in your scene." image="/img/playgroundsAndNMEs/divingDeeperShadows15.webp"/>
...
As you can see, there is now a bit of acne appearing on the object where the surface is parallel to the light direction:

![Parallel acne](/img/how_to/shadows/paralellacnea.webp)
...
Try changing the shadow generator to contact hardening: <Playground id="#FH3FM2#82" title="Self-Shadowing Step 6" description="Simple Example of adding contact hardening to your shadow generator." image="/img/playgroundsAndNMEs/divingDeeperShadows18.webp"/>

At first, you cannot see the contact-hardening effect and, not only that, you can see shadow acne again. Taking note of the section on PCSS, you realize that the light min and max should be set as close as possible: <Playground id="#FH3FM2#76" title="Self-Shadowing Step 7" description="Adjusting the light min and max for better PCSS." image="/img/playgroundsAndNMEs/divingDeeperShadows19.webp"/>

Now the contact-hardening effect is present, but the acne is even stronger. Unfortunately, the bias is applied to the normalized coordinate depth (0-1), so changing the near and far values of the light affects how large the bias should be.

So, go back and change the bias to its maximum before seeing peter panning, then apply some normal bias to remove the rest of the acne. This leads to the following result: <Playground id="#FH3FM2#83" title="Self-Shadowing Step 8" description="Dialing it all in." image="/img/playgroundsAndNMEs/divingDeeperShadows20.webp"/>

Your shadows are now soft, without acne or peter panning.

## Custom shadow map shaders

### Global custom shadow map shader

Starting with Babylon.js v4.0, you can specify your own shader to render shadow maps. To define that shader, you can use the `shadowGenerator.customShaderOptions` property:
...
Shadow map generation is a complex task and requires taking into account several defines (like the type of shadow map, between int and float, or the need for alpha test). It is recommended to check the current default shaders here:
...
To update your own uniforms, you can rely on the `shadowGenerator.onBeforeShadowMapRenderObservable` observable. This will be called for you every time the shadow map is rendered, and it will give you the current compiled effect.
...
It works for any type of materials, being a `CustomMaterial`, `PBRCustomMaterial`, `ShaderMaterial` or `NodeMaterial` instances. There's no point in using this for `StandardMaterial` and `PBRMaterial` materials because the standard shadow map shader already handles the types of deformation / alpha rejection those materials can generate (namely morph targets / bones / alpha testing).

In this demo: <Playground id="#PNQRY1#67" title="Shadows with Various Materials" description="Simple example of shadows working with different materials." image="/img/playgroundsAndNMEs/divingDeeperShadows24.webp"/>
...
For node materials, you need to tell the wrapper the variable name that holds the final world position of the vertex, as it is needed for the shadow depth computation.
...
After loading the node material, this snippet gets the variable name associated with the output of the block named `worldPosFinal` and passes it to the constructor of the wrapper.
...
The blocks can be injected at any location in your shaders if you use these `#define`s in your base material to locate the spots where the code should be injected:
...
If they are not used, the code is injected by default just before the end of each shader for the `METRIC` and `FRAGMENT` blocks, and:
...
For custom shader code written through a `ShaderMaterial` class, if you don't declare a `#define SHADOWDEPTH_NORMALBIAS` somewhere in your vertex shader code, the normal bias code **won't** be injected and so the normal bias feature won't work (it won't bug, however; it will simply do nothing). The same goes for soft transparent shadows: if you don't declare a `#define SHADOWDEPTH_SOFTTRANSPARENTSHADOW` somewhere in your fragment shader code, soft transparent shadows won't be available.
...
If you create a custom `ShaderMaterial` and need to support this functionality, you need to put `#define SHADOWDEPTH_NORMALBIAS` somewhere in your vertex code.
...
It means some extra code can still be executed in the vertex/fragment shaders (depending on where you inject the blocks of code), and all the uniforms / samplers used by the full material are still bound (even if not used) when the depth material is used for rendering. Also, on the JavaScript side, a fair amount of code is involved in checking that a standard / PBR material is ready because it must deal with all the options provided by those materials, code that could be avoided if you used a `ShaderMaterial` to implement the depth shader: that's what a standalone shadow depth wrapper is used for.
...
- the wrapper bound to `sphereFloating.material.shadowDepthWrapper` is a wrapper generated around a specific `ShaderMaterial` material that implements only the logic for fragment discarding, so it is much lighter than the shader that would be generated if it were based on the `CustomMaterial` base material
