...
The ShaderMaterial object has the necessary methods to pass data from your scene to the Vertex and Fragment Shaders and creates a material that can be applied to any mesh. This material affects how the mesh looks based on the code in the shaders.
...
- `route`: The route to the shader code. It can be any one of these:
  ...
  3. object: `{ vertexSource: "vertex shader code string", fragmentSource: "fragment shader code string" }`, used with strings containing the shader code.
...
- `options`: An object containing `attributes` and `uniforms` arrays, with their names as strings.
...
where the set method's name depends on the type.
...
When working with shaders, it is very useful to have a debug tool that can show the values passed to the shaders. For that purpose, you can use the Babylon Spector.js tool: https://github.com/BabylonJS/Spector.js, which works for any shader code, not just Babylon.js.
