...
Note that in Babylon.js, this is a [WebGPU](/setup/support/webGPU)-only feature (starting at v5.0). WebGL does not support compute shaders.
...
Use the `ComputeShader` class to create a compute shader, much like you would use `ShaderMaterial` to create a material for some custom shader code:
```javascript
...
```
`copyTextureComputeShader` is the shader code, and `bindingsMapping` is an object that maps an input variable name (from the shader source code) to its binding location. See below for more explanations.
...
* **uniform buffer**. It's a buffer you can create by instantiating the `UniformBuffer` class and use to pass some constant values to the shader side (values that can still be updated during your program—they are constants inside the shader). Note that you need to first create the layout of this buffer by calling the `addUniform` method **in the order the properties appear in the buffer in the shader code**! This point is important because the layout you create must match the layout of the buffer as used in the shader code. Once you have created the layout, you can set some values through calls to the `updateXXX()` methods. Once you are ready to update the buffer on the GPU side, call `update()`. Use `ComputeShader.setUniformBuffer()` to pass a uniform buffer to the shader.
...
Browsers do not currently support reflection for WGSL shaders, which means we are not able to automatically retrieve the binding and group values of the input variables, as seen here:
```wgsl
...
```
Note that for a sampled texture variable such as `src` in the example above, you can instruct the system to automatically bind the sampler corresponding to the texture. To do that, you must declare the sampler with a binding value equal to the binding value of the texture minus 1, and you should pass `true` as the 3rd parameter of the `setTexture()` call (or don't pass anything, as `true` is the default value). In this case, you must **not** add this sampler to the `bindingsMapping` object.

If you don't need or want to auto-bind the sampler corresponding to a texture, you can instruct the system not to bind the sampler by passing `false` as the 3rd parameter to `ComputeShader.setTexture()`.

You can also bind your own sampler by creating one (see the [TextureSampler class](/typedoc/classes/babylon.texturesampler)) and using the `ComputeShader.setTextureSampler()` method:
```javascript
const cs1 = new BABYLON.ComputeShader("myCompute", engine, { computeSource: copyTextureComputeShader }, { bindingsMapping:
    {
        "dest": { group: 0, binding: 0 },
...
<Playground id="#3URR7V#183" engine="webgpu" title="Simple compute shaders" description="A simple example of creating various compute shaders" image="/img/extensions/webgpu/simpleComputeShader.webp"/>

This PG creates 3 compute shaders:
* the first one loads a texture and copies it into another texture by means of a compute shader. This texture is then applied to the ground. **Warning**: it is for demonstration purposes only. Don't use this compute shader in real code; a simple copy buffer would be enough to achieve the same thing!
...
<Playground id="#3URR7V#273" engine="webgpu" title="Blur compute shader" description="This example shows how to blur an image using a WebGPU compute shader" image="/img/extensions/webgpu/blur.webp"/>
...
<Playground id="#3URR7V#186" engine="webgpu" title="Boids compute shader" description="A GPU compute particle simulation that mimics the flocking behavior of birds" image="/img/extensions/webgpu/boids.webp"/>
...
<Playground id="#C90R62#16" engine="webgpu" title="Hydraulic erosion" description="Simulate erosion to make terrains look more natural" image="/img/extensions/webgpu/hydraulicErosion.webp"/>
...
Note that this sample also works in WebGL2, where compute shaders are not available, but you should be careful when setting the parameters: don't raise **Iterations**, **Radius**, **Max lifetime**, or **Resolution** too much, or you may freeze your browser, because the terrain generation and erosion processes are handled on the CPU side.

### Slime simulation

<Playground id="#GXJ3FZ#51" engine="webgpu" title="Slime simulation" description="Simulate organic-like agents" image="/img/extensions/webgpu/slime.webp"/>
...
The implementation in WGSL is a little less elegant than the HLSL one because, at the time of this writing, WebGPU does not support read/write textures, so we had to use a storage buffer for the `TrailMap` texture. That means we need buffer-to-texture and texture-to-buffer copy functions, and we have to do four reads from `TrailMap` instead of a single one when we need to get a `vec4` (see the code), which is likely less performant than its HLSL counterpart.
...
By default, `StorageBuffer.read` uses the `Engine.onEndFrameObservable` observable to get the data from the buffer. This means a render loop must be set up, otherwise `Engine.onEndFrameObservable` will never be notified.
