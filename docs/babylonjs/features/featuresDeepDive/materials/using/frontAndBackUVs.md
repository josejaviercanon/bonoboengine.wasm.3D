...
Having a front and a back makes the most sense when talking about flat meshes such as a plane, a disc, or a polygon. All of these have exactly two sides, both of which can be seen only when the option _sideOrientation_ is set to **BABYLON.Mesh.DOUBLESIDE**.
...
To have different textures on the front and back, the front and back images must be in the same file, like the one below.
...
Both frontUVs and backUVs have the form Vector4(u0, v0, u1, v1), with 0&lt;= u0, v0, u1, v1 &lt;= 1, where
(u0, v0) are the bottom-left coordinates and (u1, v1) the top-right coordinates of the clipping rectangle
of the image.

To split the image above, you can define two variables:
...
and then place them in the options
...
taking into account the width-to-height ratio of the section of the image.

You then simply create a _StandardMaterial_ and set the texture to the image.
