...
To define a multi-material, you first have to define some standard materials:
...
You are now able to assign the multi-material to your mesh:
...
You can specify which part of the mesh uses a specific material by using the _subMeshes_ property. By default, every mesh comes with only one submesh that covers the entire mesh.
...
- The index of the first index to use and the index count
- The parent mesh
...
When you [merge meshes](/features/featuresDeepDive/mesh/mergeMeshes) together, setting the final parameter _multiMultiMaterial_ to true, the `subMeshes` array is automatically created with all of the merging meshes' subMeshes. Each subMesh's material is also included in the resulting mesh's new multiMaterial. This feature ignores the parameter (`subdivideWithSubMeshes`).
...
When you form `mergedMesh` by merging meshes in this array order [mesh1, mesh2], and the multiMaterial's `subMaterials` array contains materials in the order [mat1, mat2], then for the subMesh from `mesh2` to have material `mat2`, you need to set
