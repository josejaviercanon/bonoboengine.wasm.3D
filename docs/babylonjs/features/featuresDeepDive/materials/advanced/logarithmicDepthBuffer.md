...
In such cases, the linear depth buffer (the default) can generate glitches when distances start reaching large numbers.
In short, the depth values are proportional to the reciprocal of Z. This gives good precision near the camera, but less precision in the distance.
...
StandardMaterials can enable the logarithmic depth buffer if the browser supports the *GL_EXT_frag_depth* extension.
