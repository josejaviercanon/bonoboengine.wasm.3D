...
It is used internally to enable new features (like using custom materials in effect layers or with the depth renderer), but it can also be used by the user to improve performance, for example.

## Generalities
Each time a mesh has to be rendered (a submesh, to be more precise, as in Babylon.js rendering is done at the submesh level), it is done in the context of a render pass, and a render pass is identified by a simple id (number): hence the name `render pass id`. There's also a name associated with a render pass id, but it is used for debugging purposes only (you can get all the render pass names by doing `engine.getRenderPassNames()` - the render pass id is the index in this array).

A number of data items (like the `effect`) used for rendering are associated with this render pass. Technically, these data are stored in `SubMesh._drawWrappers`: this is an array of `DrawWrapper`, and the index in this array is the render pass id.
...
```javascript
const renderPassId = engine.createRenderPassId("My pass"); // the label is optional, as it is used for debugging purposes only
[...]
engine.releaseRenderPassId(renderPassId);
```

The main reason to manage render pass ids yourself is to improve performance when a mesh must be rendered with different materials depending on the rendering path.

### Changing materials when rendering to the final framebuffer
For example, if during the course of the program you must switch between Material A and Material B several times, you could simply loop over the meshes and change their `material` property. However, this is taxing on performance because each time you update the `material` property, some caches must be recreated, and it can have a visible impact (delay) on rendering.
...
Now, if you are using two render pass ids, one for each case, and set the `camera.renderPassId` property to the right one when clicking the buttons, you will notice there's no delay anymore (except the first time you click **material 2**, because the effects must be created):
...
Before 5.0, it was not easy or performant to use a different material when rendering a mesh into a custom `RenderTargetTexture` and when rendering it into the final framebuffer. Thanks to the new render pass ids, you can now simply call `RenderTargetTexture.setMaterialForRenderPass(meshOrMeshes, material)` to render a mesh in the render target texture with a specific material without any performance penalty!
...
For example, you will need a custom material to render a mesh with the depth renderer if you are using a non-standard material to display the mesh in the final framebuffer (for example, if you update the vertex positions in the vertex shader):
...
The same applies to the glow/highlight layers: use the `EffectLayer.setMaterialForRenderPass(meshOrMeshes, material)` method to set a specific material to be used for this layer.
