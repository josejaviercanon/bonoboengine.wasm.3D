...
### `HTML3DElement`
When using the `HTML3DElement` (`<babylon-viewer>`), the same options can again be passed to the constructor. However, an `HTML3DElement` instance is typically created by placing a `<babylon-viewer>` element within an HTML page, so constructor arguments cannot be passed. When it is necessary to provide options for the `HTML3DElement`, use the `ConfigureCustomViewerElement` function to create a custom element with a unique name and the desired options. For example:
...
The low-level `Viewer` exposes many protected APIs and allows multiple models to be loaded simultaneously.
