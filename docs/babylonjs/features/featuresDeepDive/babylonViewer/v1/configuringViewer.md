...
The default configuration will be loaded by default unless another configuration type is forced on the viewer by the developer. To force a different type of configuration, add the DOM attribute `extends` to the HTML element. For example:
...
The default configuration types can be overwritten using the methods described below. As the configuration object never contains arrays, any attribute in it can be referenced using a string and can therefore be overwritten.
...
1. The `extends` configuration (for example `default` or `minimal`). This is merged with the DOM configuration, giving the DOM configuration priority.
...
First, the configuration of the DOM element will be read (represented as JSON for better understanding):
...
Notice that although the minimal configuration defines antialiasing as `true`, the configuration provided by the user will be used. It only extends and does not overwrite.
...
DOM attributes are usually written in kebab-case ('looks-like-this'), mainly because some browsers convert attributes to lowercase. Those attributes will be automatically converted to camelCase ('looksLikeThis').
...
The DOM is read once and is not continuously processed. Changing the value of an attribute after the viewer is initialized will not affect the viewer.
...
As previously explained, the viewer configuration has a `configuration` member that can contain an external JSON file, which will be loaded and merged with the selected configuration.
...
The file `http://example.com/viewerConfig.json` will be loaded into the configuration object. This is a great way to create a general/global configuration for a website while providing the detailed model data using HTML.

### Configuring using JavaScript

As discussed further in [Advanced usage](//doc.babylonjs.com/extensions/Advanced_usage), JavaScript can be used to initialize a viewer. When you initialize a viewer using JavaScript, you can provide the initial configuration that will be used with that viewer:
...
As shown, the Babylon viewer has three types of configuration parsers for you to choose from: "html", "dom", and "json". Those mappers are registered in the mapper manager, which is exposed in the BabylonViewer namespace.

The mapper manager can be extended. If you have your own type of configuration or want to support additional file types, you can create your own mapper and register it. Later, you can define which mapper type the configuration will be read with by using the `configuration.type` variable in the viewer's configuration.
...
The configuration attribute of your HTML element can contain a URL, as shown earlier, or a payload of any type if you already have the configuration loaded on the page in your own format.

This is useful if you want to handle the download yourself or if the data is already available. As a best practice, you will need to:
...
This will use the pre-registered "form" mapper (which does not exist in reality; you should implement it) to read the payload and adjust the configuration.
