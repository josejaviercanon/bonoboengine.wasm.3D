...
If your scene has a lot of meshes, it can be useful to create an octree to speed up visible mesh selection (e.g. finding meshes the camera can see). To do so, just call this function:
...
* ```mesh.useOctreeForRenderingSelection``` : An octree for submeshes can even be used during mesh selection based on the camera field of view. Once a mesh is selected by the camera, the camera has to select which submeshes are visible. In this case, having an octree can be really helpful.
...
By calling ```groundMesh.optimize(chunkSize)``` where chunkSize defines the number of submeshes you want, the mesh will be optimized for rendering, picking, and collisions by creating an internal octree (be sure to select an appropriate chunkSize).
...
These functions return a [SmartArray](https://github.com/BabylonJS/Babylon.js/tree/master/packages/dev/core/src/Misc/smartArray.ts) where duplicates are present or not according to the ```allowDuplicate``` parameter.
