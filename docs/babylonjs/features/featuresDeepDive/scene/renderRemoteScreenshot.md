...
Capturing images of a Babylon scene (e.g. a GLTF model) from the server side can be useful for scenarios such as:
...
## Set up a Babylon scene you would like to render

Create a local server hosting the scene or use a Playground, e.g. <Playground id="#PN1NNI#1" title="Initial Setup" description="Simple example showing how to set up youur scene."/>
...
Typical server virtual machines do not provide access to a GPU, and when they do, setting up the proper drivers so Puppeteer can access it can be difficult. One method that has worked is using an [Azure NV virtual machine running Windows and installing grid drivers](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/n-series-driver-setup#nvidia-grid-drivers).

Once the VM is set up, Puppeteer must be configured to run in non-headless mode so that it uses the GPU renderer.
```
...
```
After this initial setup, follow the same steps described above.
