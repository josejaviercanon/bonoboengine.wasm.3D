...
The `CreateScreenshot` functions can be used to render a screenshot of your scene, allowing you to download it or even use it as a texture in the scene itself. The available methods are the following:
...
- A `{width: number, height: number}` with the width and height of the canvas and the created screenshot.
...
The success callback is a function that contains the screenshot as a string of base64-encoded characters. This string can be assigned to the src attribute of an `<img>` to display it, or it can be used to create a new Texture in the scene.
...
This function also has a `CreateScreenshotAsync` version that can be awaited using the [await](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/await) keyword.
...
As the name suggests, this function renders the scene to a [RenderTargetTexture](/features/featuresDeepDive/postProcesses/renderTargetTextureMultiPass) for the screenshot. It has a few more [configuration options](/typedoc/classes/BABYLON.Tools#CreateScreenshotUsingRenderTarget):
...
The `customizeTexture` callback can be used to modify the texture before taking the screenshot. This can be used, for example, to modify its `renderList` property, changing which objects will appear in the screenshot. You can also turn post-processes [on or off](/features/featuresDeepDive/postProcesses/usePostProcesses).
