...
An offscreen canvas is a canvas that can be rendered offscreen and on a different thread than the main UI thread.
...
The worker code will then listen for your message, get the offscreen canvas, and start Babylon.js as usual:
...
Furthermore, Babylon.js will not be able to handle inputs for you, so APIs like `camera.attachControls()` will not work and you will have to message inputs to workers.

Here is an example of communication you will have to set up to support canvas resizing.
