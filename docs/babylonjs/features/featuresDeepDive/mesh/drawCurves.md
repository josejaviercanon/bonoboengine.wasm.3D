...
If you want to draw a circular path, it is easy enough to generate the points in the XY plane using:
...
If you want to, you can work out some quite complex paths by hand.
...
Given three points in space that are not in a straight line, it is always possible to join them with a circular arc or to draw a circle through all three points.
...
- **destination** : _Vector3_ the destination point,
- **nb_of_points** : _number_ the desired number of points in the final curve array.
...
- **nbPoints** : _number_ the desired number of points in the final curve array.
...
This allows the interpolation of quaternions for use in animation. As a quaternion is a 4D object, how can you visualize the Hermite quaternion spline used by the interpolation? By representing the hyperspline in 3D space.

This is achieved by mapping a rotation quaternion onto a 3D vector.

### The Math

What is needed is a function $f$ from rotation quaternions to 3D vectors of the form
...
It follows that the set of rotation quaternions $(x, y, z, w)$ where $x^2 + y^2 + z^2 + w^2 = 1$ and $-1 \leq x, y, z, \leq 1$ and $0 \leq w \leq 1$ covers all possible rotations.
...
The process can be reversed: the inverse function $f^{-1}$ returns the rotation quaternion from a point $(x, y, z)$ on the shells.
...
**Warning**  
Using BABYLON.Quaternion.RotationAxis(axis, angle) to create any of p1, t1, p2, or t2 does not produce the expected results. Other ways of producing rotation quaternions, apart from creating them directly, should also be checked to ensure the resulting quaternion is in the range required for the mapping to work.

To produce a vector on the outer shell, you need a rotation quaternion with $w = 0$.
...
and while this may be an equivalent quaternion, it places the vector at the center of the shells, not on the outer shell.  
**End Warning**
...
This playground allows you to drag representations of the start (green) and end (red) quaternions within the unit sphere in 3D space. The representations of the start and end quaternion tangents (purple) are attached to the respective start and end controls. The tangents can also be dragged around the invisible sphere shells centered on the start and end controls. The shells, and hence the radius, can be adjusted using the up and down arrow keys (or w and x) for the selected control. While dragging or adjusting the radius, the camera is detached. The camera is reattached whenever dragging ends, and when using keys you can reattach it by pressing the spacebar.

The representation of the spline in 3D space is drawn as you adjust the controls. To apply the created Hermite quaternion spline to the box, click the animate button.
...
Because the _Curve3_ object has a useful method, _continue()_, that allows you to place the start of one _Curve3_ at the end of another _Curve3_ without any calculations to match the start and end points of the curves.
...
If you then need to draw the curve or use it for anything else, just get the array of points with the _getPoints()_ method. This method simply returns an array of successive _Vector3_.
...
Here is an example where a Hermite spline is used to smoothly close a concatenation of two Bezier curves. As the spline closes the curves, the first and last points of the open continued curve need to be read from the array.

- The first and last points of the concatenation are used as last and first point of the Hermite spline.
- The first and last segments of the concatenation are used as the last and first tangent vectors of the Hermite. Since these segments are quite small, they are scaled according to the concatenation _length_, so the longer the concatenation, the more curved the spline.
...
In light blue, these two curves are connected to each other, and a Hermite curve is also added to close the path.
