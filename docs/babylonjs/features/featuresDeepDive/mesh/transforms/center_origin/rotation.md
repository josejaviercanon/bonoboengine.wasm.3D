...
To help visualize orientation, this asymmetric mesh is used in the examples.

![The Pilot](/img/how_to/Mesh/pilot.webp)

The most straightforward method to change the orientation of a mesh is the rotation property.
...
```
where alpha, beta, and gamma are angles measured in **radians** and are known as the **Euler angles**.

Four questions need immediate answers:
...
The answer to 4 depends on 3, and in Babylon.js the rotation frame of reference is the local space of the mesh being rotated.
...
mesh.rotation.x  =  alpha;
```
The order is as follows: a rotation of beta about the local y axis, then alpha about the local x axis, and finally a rotation of gamma about the local z axis.
...
However, sometimes you want to rotate around axes other than the x, y, and z axes in local space and define a rotation sequence of your own.