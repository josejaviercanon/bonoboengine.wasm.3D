...
There are several methods of achieving rotations within Babylon.js, and each uses a particular convention.


## Euler Angles

In 3D space, Euler angles can produce any possible orientation by providing three angles to rotate about three axes in a given order.  

For the three axes X, Y, and Z, there are 12 different permutations for the order of the angles. Since X, Y, and Z can be in *World Space* or in *Local Space*, this means there are potentially 24 different possibilities. Most, if not all, of these are used in different systems around the world. So you need to be very clear about the convention used by the system you are working in.
...
Applying independent rotations to a newly created mesh (i.e. one that has zero rotations) in the order YXZ using local axes
...
which will produce this orientation regardless of the mesh's orientation before it is applied. The playground below demonstrates this by randomly generating angles and then applying these two methods to two different boxes, which remain in alignment.
...
Applying independent rotations to a newly created mesh (i.e. one that has zero rotations) in the order ZXY
...
As a reminder, this convention is used directly in the _rotation_ method in Babylon.js in the form

```javascript
mesh.rotation = new BABYLON.Vector3(pitch, yaw, roll);
```

In this convention, you take the three angles, yaw, pitch, and roll and rotate yaw about Y, then pitch about X, and roll about Z using the **local** axes.
...
Applying the above _rotate_ sequence to a newly created mesh (i.e. one that has zero rotations) in the order ZXZ in **world** space, and applying _RotationAlphaBetaGamma_ to a mesh with any orientation using the same angles, will produce the same orientation. The playground below demonstrates this by randomly generating angles and then applying these two methods to two different boxes, which remain in alignment.

<Playground id="#1ST43U#53" title="ZXZ To Quaternion" description="Simple example of converting ZXZ to quaternion."/>


So far, we have considered one mesh operating in its local space and in Babylon.js world space. What if we want one mesh to operate in the frame of reference of another mesh? Before we consider parenting in the next section, we will look at ways this can be done using the data that Babylon.js directly stores for a mesh's transformations and coordinates.