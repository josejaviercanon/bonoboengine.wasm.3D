...
```
Like the *rotation* property, the *rotationQuaternion* property sets the orientation of the mesh with the local origin as the center of rotation.
...
Any rotation quaternion can be converted to Euler angles for use with *mesh.rotation*:
...
## Warning
You cannot use a *rotationQuaternion* followed by a *rotation* on the same mesh. Once a *rotationQuaternion* is applied, any subsequent use of *rotation* will produce the wrong orientation unless the *rotationQuaternion* is first set to null. Please be aware that this often applies when importing models, as many of them already have a *rotationQuaternion* set. 

From version 4.0 onward, `rotationQuaternion` is set to null automatically when, and only when, *rotation* is set directly with a vector, for example:
```javascript
