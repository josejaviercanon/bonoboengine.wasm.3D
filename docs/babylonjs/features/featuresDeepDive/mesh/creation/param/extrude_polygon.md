...
image: 
description: Learn how to extrude irregular polygons in Babylon.js.
...
*ExtrudePolygon* adds a *depth* option to *CreatePolygon*. The extrusion is carried out down the vertical y axis from the polygon shape in the horizontal xz plane. Like *CreatePolygon*, it requires an Earcut script. Since it can be concave, it requires a way to fill the polygon with triangles, and an Earcut method is used. Although an Earcut script is pre-loaded in the Playground, you will have to add a reference to such a script in your own projects. One is available from the [CDN](https://unpkg.com/earcut@2.1.1/dist/earcut.min.js) or via a [npm package](https://github.com/mapbox/earcut#install).
...
All vectors for shape and holes are Vector3 and should be in the xz plane, i.e. of the form BABYLON.Vector3(x, 0, z), and in **counter-clockwise** order.
