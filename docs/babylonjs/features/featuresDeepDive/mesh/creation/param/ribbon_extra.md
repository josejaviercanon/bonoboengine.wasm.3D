...
Let's take a closer look at a ribbon.

It is formed using paths. A path is simply an array of a minimum of two vector3 points.

The image below shows the construction of a ribbon using two paths, A and B, each having five points.
...
You can have as many paths as you wish, including just one. You can close all the paths and/or close the paths array itself. Imagine a long, narrow ribbon in the real world with wire running down its length. Closing the paths forms a loop of ribbon, while closing the array forms a tube.
...
The ribbon is formed by joining each point on the path to a later point where one exists. The _offset_ property governs how many points ahead the current point will be joined to. The triangular facets for the mesh are formed from the current point, the next point, and the offset point.
...
The best way to emulate different lengths for some parts of your mesh is simply to use many ribbons.
...
As you can see, the final ribbon adjusts to different lengths. The rule is that they all start from the first path points, and each intermediate ribbon then stops at the first end reached by either of its two constituent paths. However, while you can add color using a material, as done here: <Playground id="#88AZQ#17" title="Create a Colored Ribbon" description="Simple example of creating a colored ribbon."/>

There is no effect on light reflection for a ribbon with different-length paths.
...
When applying a texture, the algorithm assumes that the ribbon can be unfolded into a rectangle that can be stretched to fit on top of the image used for the texture.
...
<Playground id="#3XMWZ#44" title="Start With An Unclosed Ribbon" description="Simple example of creating an unclosed ribbon."/>
...
you just have to forget automatic closing and close the ribbon yourself. A simple way to do this is to push the first _path_ again at the end of the _pathArray_.
