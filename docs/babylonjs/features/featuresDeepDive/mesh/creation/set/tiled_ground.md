...
A tiled ground is created differently from a ground mesh. It still lies in the xz plane. The bottom-left corner of the tiled ground is given by the values for xmin and zmin, and the top-right corner by xmax and zmax.
The tiled ground is subdivided into tile regions: across into w tiles and up into h tiles. In the same way, every tile can be further subdivided into w by h sections. Creating a tiled ground relies on the use of _MultiMaterials_.
...
| zmax         | _(number)_ map max z coordinate value                                                                         | 1              |
...
- Create a multi material and push each material onto its subMaterials array
