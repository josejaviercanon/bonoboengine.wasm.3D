...
The entire system has been split into two parts: the Smart Filter and the Smart Filter Runtime.
...
During the initialization phase, the `SmartFilter` examines the graph and builds a list of commands to execute during each frame, storing them in a `CommandBuffer`:
...
- Each block is responsible for "injecting" its command into the Smart Filter's `CommandBuffer`.
...
Each block is responsible for exposing its inputs and outputs through `registerInput` and `registerOutput`:
...
- Strongly typed, allowing more type safety while creating a `SmartFilter` in code, yet keeping enough flexibility to be understood efficiently at runtime.
- They are also responsible for their compatibility when linked to each other.

The Smart Filter is fully abstracted away from the runtime and does not even require a Babylon Engine to work with. It is only responsible for holding the "graph" of the Smart Filter or the "map" of all its blocks. A Smart Filter only needs a name to be created:
...
To keep a clear separation between edit time, resource management, and render time, the filter itself cannot be rendered directly. To do so, a runtime needs to be created from the filter:
...
The runtime contains the list of resources required to render the filter, such as intermediate textures, shaders, and buffers. It also owns a CommandBuffer containing the list of all the actions required to render the filter.
...
Whereas a more complex one could look like:
...
The command buffer is accessible through the runtime for debugging and logging purposes. The list of commands can be extended with custom ones if necessary, such as for introspection as we do in ./src/command/commandBufferDebugger.ts.
