...
Examples on these pages can be viewed in the Playground, the place to try out Babylon.js live on the web, by clicking their titles. To edit the code, open them with ![open pg](/img/getstarted/openpg.webp?expandable=false)
...
Wait... what is the Babylon.js Engine, you ask? Excellent question. The `engine` variable shown below is the class responsible for interfacing with lower-level APIs such as WebGL, audio, and more. The constructor used to create a Babylon scene (the context that renders visuals to the screen) needs the engine to talk to these lower-level APIs. This is why the `engine` variable is needed when creating a scene.
...
Now that we have a file, we can use it to demonstrate how to view it on a web page.
