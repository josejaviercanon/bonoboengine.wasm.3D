...
This documentation explains how to transform an existing Babylon.js game into an Instant Game.
...
3. Then you have to initialize the instant game with the `FBInstant.initializeAsync` method. In the game example, I created a file called `main.ts` that creates the game once the framework is initialized:
...
4. Update the Facebook loading screen: by using an Asset Manager, you can use the exposed `onProgress` property to update the loading screen.
...
6. Once your game is ready, you can upload it through the Facebook Instant Games Dashboard and run it. Setup is complete.

## Example

An example game can be found here: [SIMON3D](https://github.com/Temechon/simon3d)

This game is far from finished, but it shows how to configure a very simple Babylon.js demo game and make it run as an Instant Game.
