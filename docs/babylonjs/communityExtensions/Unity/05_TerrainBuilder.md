...
The details of all the tools are given in subsequent sections. However, they are all based on the concept of painting detail and, with the exception of the tree placement tool, all use the same options for brushes, brush size, and opacity. The tools can be used to set the height of the terrain and to add coloration, plants, and other objects.
