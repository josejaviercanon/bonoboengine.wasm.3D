...
It has been noticed that sometimes the Editor cannot be started (or starts with WebGL errors) and therefore cannot be
used. This problem has been noticed mostly on devices using Intel HD Graphics.
...
- In `Graphics Performance Preference`, select `Desktop App` in the drop-down and then browse to `BabylonJSEditor.exe`. You should now see it added to the list.
...
Read the caveat, but in short: your default drivers are usually customized by the manufacturer, but this generic driver is not. Obviously, you run the risk of breaking something.
