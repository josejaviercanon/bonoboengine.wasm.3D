...
Using the inspector, collision properties can be customized per object:
...
To enable collisions, simply check the `Enabled` checkbox and provide the desired gravity values for the current scene.

## Enabling Collisions For Camera
Cameras do not necessarily check for collisions by default. If your camera does not check for collisions, simply select the desired
camera in the graph and scroll in the inspector to find the section named `Collisions`.

*Note: According to the selected camera type, not all properties will be shown. For example, the `Apply Gravity` option will not be shown
for `Arc Rotate Camera` as it makes no sense.*

### Free Camera
The `FreeCamera` type is the most common camera type in projects. To enable collisions, simply check the
...
and set up the desired values according to the nature of the current scene being edited in the Editor.

The `Apply Gravity` property, if enabled, applies the gravity previously set in the scene and can be disabled only by unchecking the
property.

![SceneGraph](/img/extensions/Editor/BasicCollisions/free_camera.webp)

### Arc Rotate Camera
As with the `FreeCamera`, arc rotate cameras can have their collisions enabled using the `Enabled` checkbox.
Unlike the free camera, the ellipsoid property is replaced by the `Collisions Radius` property. Check the
[following documentation](/features/featuresDeepDive/cameras/camera_collisions#arcrotatecamera) in order to understand and set up
...
Once the scene and camera(s) have been configured, the last step is to configure which mesh(es) will have collisions enabled in order
to block the camera.
...
Do not hesitate to also check the
...
The Babylon.JS API provides a method named `.moveWithCollisions` on the `Mesh` class. This method works like it does for cameras without any
...
- In the current frame, it will recompute the world matrix on the current node and all of its parents, so the complexity is O(d), where d is the depth of NodeN.
- On the next frame, when `computeWorldMatrix` is called by default by the render loop, all children of NodeN will see that the node is not synchronized with its parent, which will force a recomputation of those nodes. Note that if we had not called `computeWorldMatrix(true)` on NodeN, then its children would skip the recalculation because they would be synchronized with the parent.

So the complexity of forcing recalculation on NodeN increases by the number of ancestors (for the current frame) and the number of descendants (for the next frame).