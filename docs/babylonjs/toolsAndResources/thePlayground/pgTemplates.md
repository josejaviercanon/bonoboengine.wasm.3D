...
With Babylon.js 4.2, you now have a handy new tool available in the Playground. This tool allows you to hit `CTRL+Space` on your keyboard to bring up handy snippets of code. From loading a box to exporting a `.gltf` file, some of the most commonly used code blocks are now available at the click of a button.
...
Each Playground template entry in this `.json` file should include three components:
...
${4:mesh_that_receives_the_shadow}.receiveShadows = true;
```
While the label and documentation components are fairly straightforward, the code snippet itself has a few things to note.
