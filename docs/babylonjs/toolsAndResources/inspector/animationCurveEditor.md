...
The Menu lets you create a new animation, load an animation from the snippet server or from a local file, save an animation locally or to the snippet server, edit an animation, and change an animation's playback framerate.
...
- Property: The list of available animatable properties if Mode is set to `List`, or a text field when Mode is set to `Custom`.
...
Pressing the edit tab will display a list of all properties with animations applied to them. Note that if you entered ACE by selecting the edit button of a Babylon object (Method 1 listed above), this menu will display ALL properties of the selected object that have animations attached to them.
...
<img src="/img/tools/ACE/frameCanvas.webp" title="Frame Canvas"/>
...
With one or more keys selected, the flatten tangents button will flatten the tangents coming into and going out of the key.

#### Creating Linear Key Tangents

<img src="/img/tools/ACE/linearTangents.webp" title="Linear Tangents"/>

With one or more keys selected, the linear tangents button will create linear tangents coming into and going out of the key.

#### Breaking a Key's Tangents

<img src="/img/tools/ACE/brokenTangents.webp" title="Broken Tangents"/>

With one or more keys selected, the break tangents button will separate the incoming and outgoing tangents of a key so they can be manipulated individually.

#### Unifying a Key's Tangents

<img src="/img/tools/ACE/unifyTangents.webp" title="Unified Tangents"/>

With one or more keys selected, the unify tangents button will unite the incoming and outgoing tangents of a key so they can be manipulated together.

#### Stepping a Key's Tangents

<img src="/img/tools/ACE/stepTangents.webp" title="Step Tangents"/>

With one or more keys selected, the step tangents button will create a flat incoming and outgoing tangent relative to the previous and following key. This is useful when blocking out positions or poses, which is common in the animation process. You can think of stepped tangents as a series of static positions with no computer interpolation between them.
...
The Canvas will display all curves for any selected property in the Edit Menu. In other words, selecting specific properties in the Edit Menu will limit the canvas to showing only those properties.

You can select a key by either left-clicking it or clicking and dragging over it.
...
If you hold down the SHIFT key and left-click a key while dragging your mouse vertically, the key will be locked from moving horizontally. This allows you to easily change a key's value without changing the frame it occupies.

Similarly, if you hold down the SHIFT key and left-click a key while dragging your mouse horizontally, the key will be locked from moving vertically. This allows you to easily change the frame number a key occupies without changing its value.

#### Navigation

Holding down the ALT key and left-clicking and dragging on the canvas will pan it around. The middle mouse button can also be used for the same thing.
...
The play forward and backward buttons will play the animation forward or backward, respectively, at the specified framerate (fps) in the Edit Menu. The play buttons will play from the starting frame of the play range to the ending frame of the play range. You can also use the `spacebar` key to play the animation forward. There is no hotkey to play the animation in reverse, because we are not animals.
...
It is very easy to load your saved animations into your Babylon.js scene and modify them. Here are a few handy examples:
...
You can load an animation into your Babylon scene from a saved `.json` file like this:
