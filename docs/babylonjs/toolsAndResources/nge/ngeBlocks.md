...
## Sources
Source nodes are required for any Node Geometry graph because they supply the geometry needed by the `GeometryOutput` node for the graph to build. Most of these nodes come with multiple inputs that set the basic properties of the geometry, but all have exactly one output, which is the geometry generated by the source node.
...
This source node holds a mesh loaded from disk or attached from the scene. This node has no inputs because all properties come from the loaded mesh. Meshes can be assigned to this node in code using [the following method](/typedoc/classes/BABYLON.NodeGeometry#getBlockByName).
``` javascript
...
#### Input
- **value** is a value that can be of any input type and will be evaluated against the values connected to edge0 and edge1 using the greater-than-or-equal logical operator.
...
#### Input
- **value** is a value that can be of any input type and will be evaluated against the value connected to edge using the greater-than-or-equal logical operator.
...
Contextual nodes typically represent arrays of values determined by components of the source, such as the vertices of referenced geometry. There are a couple of common properties on all contextual nodes:
...
## Math: Standard
These nodes are simple mathematical operations like add, subtract, min, max, or round. The nodes in this section are some of the most used nodes in any of our node graph tools. They are very straightforward, but many of the nodes in this set have features that are worth calling out specifically.
...
#### Outputs
- **output** is the sum of the left and right inputs and matches the type of the values connected to `left` and `right`.
...
#### Outputs
- **output** is the clamped value from the input and matches the type of the value connected to `input`.
...
#### Outputs
- **output** is the quotient of the left input divided by the right input and matches the type of the values connected to `left` and `right`.
...
#### Outputs
- **output** is the larger value between the left input and the right input and matches the type of the values connected to `left` and `right`.
...
#### Outputs
- **output** is the smaller value between the left input and the right input and matches the type of the values connected to `left` and `right`.
...
#### Outputs
- **output** is the remainder of the Euclidean division of the left input by the right input and matches the type of the values connected to `left` and `right`.
...
#### Outputs
- **output** is the product of the left input multiplied by the right input and matches the type of the value connected to `left`. The right input does not influence the output type in any way.
...
This is a simple function to return the principal square root of a nonnegative value connected to the input port. This node can support Float, Int, Vector2, Vector3, and Vector4 types. The type for the node connected to the input of this node determines the type for the node output.
...
This is a simple math node which subtracts the right input from the left input. This node can support Float, Int, Vector2, Vector3, and Vector4 types, though the node cannot subtract mixed types. The type of the node first connected to an input determines the type for the node output.
...
#### Outputs
- **output** is the difference of the right input subtracted from the left input and matches the type of the values connected to `left` and `right`.

## Math: Scientific
These nodes are a collection of more complex mathematical functions, mostly trigonometric. They are all very straightforward nodes with one input and one output. All nodes in this set can support Float, Int, Vector2, Vector3, and Vector4 types.
...
- **source** provides float positional values.
...
Finally, to fully get a sense of what the lattice node can do, try adjusting a few things. Try increasing the subdivisions of the Cylinder geometry. After playing with the 'LatticeYInfluence' slider again, you'll notice the geometry change differently. Then try increasing the resolution of the Lattice Cage in the Lattice node. Again, you will see different results.
...
When working with geometry, it is often necessary to create a new UV mapping for it, such as when the geometry has been optimized or when an additional UV set is desired. This node will create a new set of UV coordinates for the geometry based on the selected projection method. The available projections are spherical, cylindrical, and cubic, and more may be added in the future. Simply pass the positions contextual value to the position input and a Vector3 to the center input to define the center of the projection and create UV coordinates for each vertex.
...
- **center** is connected to a node providing a Vector3 position for the center of the projection. This allows for more precise mapping by letting the projection shape be moved to encompass the mesh in the way that will best unwrap it.
...
In this example, a box is subdivided into a spherical shape: <NGE id="#X7L4SK" title="Subdivide Example" description="Simple example of the Subdivide Node."/>
...
#### Properties
- **offset** is an input field for a Vector3 value to be used to offset the noise texture. This property is only visible if there is no wire connected to the offset input.
- **scale** is an input field for a Float value used to scale the noise texture. This property is only visible if there is no wire connected to the scale input.
- **octaves** is an input field for a Float value to be used to adjust the number of octaves in the noise texture, which increases the frequency of the pattern and results in increased detail in the texture. This property is only visible if there is no wire connected to the octaves input.
- **roughness** is an input field for a Float value to be used to adjust the amplitude of the octaves in the noise texture, which adds contrast to the texture. This property is only visible if there is no wire connected to the roughness input.
...
#### Inputs
- **offset** is connected to a node providing a Vector3 value to offset the noise texture.
...
## Textures
Texture data can be loaded and cached to be made available to the graph. Texture data can be used for operations like instantiating meshes based on texture values at a given UV coordinate or assigning vertex color to a mesh. Using textures in the graph requires both a Texture node and a Texture Fetch node. The Texture node takes care of loading and caching the texture data, while the Texture Fetch node returns specific texture data based on UV coordinates. Unlike Texture nodes in the Node Material Editor, texture data is not rendered on the mesh directly, but is instead used for driving instantiation or setting mesh properties.
...
This node is required to create the geometry, and whatever is connected to this node is the final geometry for the graph. This node does not have any special considerations for use, but there are some useful things to know about it. Selecting the node will display useful information about the rendered geometry output, including vertices, faces, build time, and sub-meshes. The node also provides stats about whether the geometry has normals, vertex colors, UVs and how many sets there are, tangents, matrix weights, and matrix indices.
