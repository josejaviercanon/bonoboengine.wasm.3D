...
image: 
description: The Babylon.js materials library ShadowOnlyMaterial displays only shadows cast by a light on a receiving object.
...
The goal of the ShadowOnlyMaterial is to display only shadows cast by a light on a receiving object. It is mostly useful when you want to display shadows on a transparent canvas (on top of your existing DOM).

The ShadowOnly material can be found here: [https://cdn.babylonjs.com/materialsLibrary/babylon.shadowOnlyMaterial.js](https://cdn.babylonjs.com/materialsLibrary/babylon.shadowOnlyMaterial.js)
...
ShadowOnly material is very simple to use. Just apply it as a material to any mesh where you want to display only shadows:
...
So if you have multiple lights in your scene, you may need to use `light.includedOnlyMeshes` or `light.excludedMeshes` or `light.includeOnlyWithLayerMask` or `light.excludeWithLayerMask`.
